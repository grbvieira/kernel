...
int callback_bbd_on_packet(void *ssh_data, const char *buf, size_t size)
{
	...
	while (idx < size)
	{
		...
		required = (remain < ssp_pkt.required) ? remain : ssp_pkt.required;
		...
		case WAITFOR_PKT_COMPLETE:
			...
			case AP2HUB_READ:
				...
				if (ssp_pkt.msg != NULL && ssp_pkt.rxlen == ssp_pkt.msg->length && ssp_pkt.rxlen < MAX_SSP_DATA_SIZE)
				{
					mutex_lock(&data->pending_mutex);
